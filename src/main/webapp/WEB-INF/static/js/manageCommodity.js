...
function ajaxSubmit(submitBtnId,formId,resetForm,callBack){
    ...
    formElement.prop("action", path+"/commodity/getAllCommodity");
...
function doAfterGetList(data) {
    ...
    var limitList = "";
    var object = data.pageCompleteCommodity;
    if (object) {
    ...
    for (var i = 0; i < limitList.length; i++) {
        ...
        var trElement = "<tr><td><input  class='childrenCheckBox' type='checkbox' value='"
            ...
            + "</td>"
            + "<td>"
            + staffLimit.goodsdesc
            + "</td>"
            + "<td>"
            + staffLimit.goodsquantity+"个"
            + "</td>"
            + "<td>"
            + staffLimit.goodsweight+"kg"
            + "</td>"
            + "<td>"
            + staffLimit.cost+"元"
            + "</td></tr>";
    ...
    //分页组件调用
    $("#detailLinkDiv").show();
    var pageMsg = data.pageCompleteCommodity;
    if(pageMsg){
        laypage({
        ...
        if(k == 0) {
            ...
            var submitForm = function() {
                ...
                for(var i=0;i<staffClassIdList.length;i++){
                    $("#form").append($('<input type="hidden" value="'+staffClassIdList[i]+'" name="commodityid" class="staffIdFromCheckBox"/>'));
                    $.ajax({
                        type:"post",
                        url:path+"/commodity/deleteCommodityById",
            ...
            $("#singleLimitFrom").prop("action", url + "?detailType=" + k);
            $("#form").append($('<input type="hidden" value="' + staffClassIdList[0] + '" name="commodityid" class="staffIdFromCheckBox"/>'));
            $.ajax({
                type:"post",
                url:path+"/commodity/getCompleteCommodityById",
                data:$(".staffIdFromCheckBox").serialize(),
                success:function(data){
                    if (data.code == "0000") {
            ...
            $("#singleLimitFrom").prop("action", url + "?detailType=" + k);
            $("#form").append($('<input type="hidden" value="' + staffClassIdList[0] + '" name="vid" class="staffIdFromCheckBox"/>'));
            console.log("staffClassIdList[0]的值为："+staffClassIdList[0]);
            $.ajax({
                type:"post",
                url:path+"/voicemail/getVoicemail",
                data:$(".staffIdFromCheckBox").serialize(),
                success:function(data){
                    if (data.code == "0000") {
                        ajax(thisElement,data,k);
...
function openWindow(title,btn1,btn2,type,datas) {
    ...
    if (datas != null) {
        if(type==1){
            ...
            $("#commodityid").val(datas.completeCommodity.commodityid);
            $("#goodsdesc").val(datas.completeCommodity.goodsdesc);
            $("#usernames").val(datas.completeCommodity.username);
            $("#goodsnames").val(datas.completeCommodity.goodsname);
            $("#goodsquantity").val(datas.completeCommodity.goodsquantity);
            $("#goodsweight").val(datas.completeCommodity.goodsweight);
            $("#cost").val(datas.completeCommodity.cost);
            $("#ordercodes").val(datas.completeCommodity.ordercode);
            $("#logisticcodes").val(datas.completeCommodity.logisticcode);
            $("#photo").prop("src",path+"img/"+datas.completeCommodity.goodsphoto);
        }
    }
