        ...
        if (k == 0) {
            ...
            var submitForm = function() {
                ...
                for(var i=0;i<staffClassIdList.length;i++){
                    $("#form").append($('<input type="hidden" value="'+staffClassIdList[i]+'" name="orderid" class="staffIdFromCheckBox"/>'));
                }
                $.ajax({
                    type:"post",
                    url:path+"/order/deleteOrders",
            ...
            $("#singleLimitFrom").prop("action", url + "?detailType=" + k);
            $("#form").append($('<input type="hidden" value="' + staffClassIdList[0] + '" name="orderid" class="staffIdFromCheckBox"/>'));
            console.log("orderid的值为："+staffClassIdList[0]);
            $.ajax({
                type:"post",
...
function openWindow(title,btn1,btn2,type,datas) {
    ...
    switch (datas.orders.orderstate) {
        case "0":
            state = "已下订单";
            break;
        case "1":
    ...
    if (datas != null) {
        $("#logisticcode").val(datas.orders.logisticcode);
        $("#ordercodes").val(datas.orders.ordercode);
        $("#date").val(datas.orders.date);
        $("#sendername").val(datas.orders.sendername);
        $("#receivername").val(datas.orders.receivername);
        $("#senderphone").val(datas.orders.senderphone);
        $("#receiverphone").val(datas.orders.receiverphone);
        $("#senderaddress").val(datas.orders.senderaddress);
        $("#receiveraddress").val(datas.orders.receiveraddress);
        $("#goodsweight").val(datas.orders.goodsweight+"kg");
        $("#goodsdesc").val(datas.orders.goodsdesc);
        $("#goodsquantity").val(datas.orders.goodsquantity+"个");
        $("#shippername").val(datas.orders.shippername);
        $("#states").val(orderState);
        $("#orderstates").val(state);
        $("#logisticcode").val(datas.orders.logisticcode);
        $("#cost").val(datas.orders.cost+"元");
        $("#senderlocation").val(datas.orders.senderlocation);
        $("#receiverlocation").val(datas.orders.receiverlocation);
        $("#goodsname").val(goodsname);
        $("#paytype").val(paytype);
        $("#remark").val(datas.orders.remark);
        $("#photo").prop("src",path+"img/"+datas.orders.goodsphoto);
    }
...
function openWindows(title,btn1,btn2,type,datas) {
        ...
        var windowUrl = path + '/order/updateOrders';
        if(datas!=null) {
            switch (datas.orders.state) {
                case "0":
                    $("#State").find("option[text='未揽件']").attr("selected", true);
                    break;
                case "1":
                    $("#State").find("option[text='已揽件']").attr("selected", true);
                    break;
                case "2":
                    $("#State").find("option[text='在途中']").attr("selected", true);
                    break;
                case "3":
                    $("#State").find("option[text='已签收']").attr("selected", true);
                    break;
                case "4":
                    $("#State").find("option[text='问题件']").attr("selected", true);
                    break;
                default:
                    break;
            }

            $("#orderid").val(datas.orders.orderid);
            //$("#logistic").val(datas.orders.logisticcode);
            $("#State").val(datas.orders.state);
        }
    ...
    layer.open({
        ...
        yes: function(index, layero){
            ...
            $.ajax({
                ...
                type:"post",
                url:windowUrl,
                data:{state:orderstate,orderid:orderid},
                success:function(data){
                    tipsMsg(data.msg);
                    if("0000"!=data.code){
                        return false;
