...
function toDetails() {
    ...
    aElement.click(function () {
        ...
        //url = url.split("?")[0];
        if (k == 0) {
            $("#form").append($('<input type="hidden" value="' + staffIdList[0] + '" name="id" class="staffIdFromCheckBox"/>'));
            /* $.ajax({
                 type: "post",
                 url: url,
            ...
            var id=$(".staffIdFromCheckBox").val();
            $("#singleStaffFrom").prop("action",path+"/orderReservation/toSingleTraceReservation?id="+id+"&detailType="+k);
