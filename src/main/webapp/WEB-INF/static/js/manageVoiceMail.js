...
function ajaxSubmit(submitBtnId,formId,resetForm,callBack){
    ...
    formElement.prop("action", path+"/voicemail/getAllVocieMails");
...
function doAfterGetList(data) {
    ...
    //分页组件调用
    $("#detailLinkDiv").show();
    var pageMsg = data.pageVoiceMail;
    if(pageMsg){
        laypage({
        ...
        if (k == 0) {
            ...
            var submitForm = function() {
                ...
                for(var i=0;i<staffClassIdList.length;i++){
                    $("#form").append($('<input type="hidden" value="'+staffClassIdList[i]+'" name="vid" class="staffIdFromCheckBox"/>'));
            }
                $.ajax({
                    type:"post",
...
function openWindow(title,btn1,btn2,type,datas) {
    ...
    if (datas != null) {
     if(type==1){
         ...
         $("#voicemaildate").val(datas.voiceMail.voicemaildate);
         $("#replydate").val(datas.voiceMail.replydate);
         $("#photo").attr("src",path+"/img/"+datas.voiceMail.photo);
         $("#content").val(datas.voiceMail.content);
         $("#reply").val(datas.voiceMail.reply);
         $("#vid").val(datas.voiceMail.vid);
     }else if(type==2) {
         $("#voicemaildate").val(datas.voiceMail.voicemaildate);
         $("#reply").val(datas.voiceMail.reply);
         $("#vid").val(datas.voiceMail.vid);
         ...
         $("#trReplyDate").hide();
         $("#tdPhoto").hide();
         $("#replydate").val(CurentTime());
         $("#windowId").find("input").prop("disabled", true);
         $("#content").prop("disabled",true);
         var windowUrl = path +'/voicemail/updateVoicemail';
     }
    }
    ...
    layer.open({
        ...
        yes: function(index, layero){
            ...
            $.ajax({
                type:"post",
                url:windowUrl,
                data:{replydate:replydate,vid:vid,reply:reply,state:state},
                success:function(data){
                    tipsMsg(data.msg);
                    if("0000"!=data.code){
                        $disabled.prop("disabled",true);
