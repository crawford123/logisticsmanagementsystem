...
function toDetails() {
    ...
    aElement.click(function() {
        ...
        if (k == 0) {
            ...
            var toUrl = path + "/order/toManageOrderTraces";
...
function openWindow(title,btn1,btn2,type,datas) {
    ...
    if (datas != null) {
        if (type == 2) {
            $("#orderid").val(datas.traces.orderid);
            $("#tracesid").val(datas.traces.tracesid);
            $("#accepttime").val(datas.traces.accepttime);
            $("#acceptstation").val(datas.traces.acceptstation);
            $("#remark").val(datas.traces.remark);
            layTitle="修改订单轨迹信息";
            var windowUrl = path + '/order/updateTrace';
        }
        if (type == 1) {
            $("#orderid").val(datas.traces.orderid);
    ...
    if(type==3){
        ...
        windowUrl =  path+'/order/addTraces';
        layTitle="新增订单轨迹信息";
    }
    layer.open({
        title:layTitle,
        ...
        yes: function(index, layero){
            ...
            $.ajax({
                type:"post",
                url:windowUrl,
                data:{orderid:orderid,tracesid:tracesid,acceptstation:acceptstation,accepttime:accepttime,remark:remark},
                success:function(data){
                    tipsMsg(data.msg);
                    if("0000"!=data.code){
                        //$disabled.prop("disabled",true);
